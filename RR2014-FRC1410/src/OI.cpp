...
OI::OI(){
	...
	toggle_intake_wheels = new JoystickButton(operator_stick, intakeInwards);
	move_tote_lifter = new JoystickButton(operator_stick, moveToteLifter);
	move_can_lifter = new JoystickButton(operator_stick, moveCanLifter);

	toggle_intake_wheels->ToggleWhenPressed(new SimultaneousIntakeOp());
	move_tote_lifter->ToggleWhenActive(new MoveToteLifter());
	move_can_lifter->ToggleWhenPressed(new SimultaneousManipulatorOp());
}

double OI::GetDriveAxis(bool left){
	if(left){
		return (InputShape(driver_stick->GetRawAxis(1)));
	}
	else{
		return -(InputShape(driver_stick->GetRawAxis(5)));
	}
}

double OI::GetArmAxis(bool lower){
	if(lower){
		return (InputShape(operator_stick->GetRawAxis(4)));
	}
	else{
		return (InputShape(operator_stick->GetRawAxis(0)));
	}
}

double OI::GetElevatorAxis(){
	return (InputShape(operator_stick->GetRawAxis(3) + operator_stick->GetRawAxis(4)));
}
