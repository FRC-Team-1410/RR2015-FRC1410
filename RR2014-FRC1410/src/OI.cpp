...
OI::OI(){
	...
	toggle_inner_in->ToggleWhenPressed(new ToggleInnerWheels(1));
	toggle_inner_out->ToggleWhenPressed(new ToggleInnerWheels(-1));
	//reset_robot->ToggleWhenPressed(new ResetRobot());
	toggle_outer_rollers->ToggleWhenPressed(new ToggleIntakeWheels());
	toggle_can_lifter->ToggleWhenPressed(new MoveCanLifter());
	toggle_tote_lifter->ToggleWhenPressed(new MoveToteLifter());
}
...
double OI::DriveArms(bool upper){
	if(upper){
		return InputShape(-operator_stick->GetRawAxis(articulate_upper_arms)/2);
	}
	else{
		return InputShape(-operator_stick->GetRawAxis(articulate_lower_arms)/2);
	}
}

double OI::GetDriveAllAxis(){
	return InputShape(operator_stick->GetRawAxis(1));
}
